...
class SessionCheerioCrawler extends Apify.CheerioCrawler {
    ...
    async _defaultRequestFunction({ request }) {
        try {
            ...
            if (
                e.message.includes("To discuss automated access to Amazon data")
            ) {
                // eslint-disable-next-line no-underscore-dangle
    ...
    _getProxyUrl(proxySession, request) {
        ...
        if (
            this.useApifyProxy &&
            this.apifyProxyGroups && this.apifyProxyGroups.length
        ) {
            return `http://groups-${this.apifyProxyGroups.join(
                "+"
            )},session-${proxySession},country-${
                country ? country.toUpperCase() : this.country
            }:${this.proxyPassword}@proxy.apify.com:8000`;
        }
        if (
            this.useApifyProxy &&
            (!this.apifyProxyGroups || this.apifyProxyGroups.length === 0)
        ) {
        ...
        if (this.proxyUrls) {
            return this.proxyUrls[
                this.lastUsedProxyUrlIndex++ % this.proxyUrls.length
            ];
        }
